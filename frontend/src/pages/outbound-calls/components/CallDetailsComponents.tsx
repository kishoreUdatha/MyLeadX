...
import {
  STATUS_CONFIG,
  OUTCOME_CONFIG,
  EXCLUDED_QUALIFICATION_KEYS,
  formatDuration,
  formatTime,
  formatDate,
  getSentimentColor,
  formatQualificationValue,
  formatQualificationKey,
  isLongTextKey,
} from '../call-details.constants';
...
export const Header: React.FC<HeaderProps> = ({ call, onBack, onViewLead }) => {
  const status = STATUS_CONFIG[call.status] || { label: call.status, color: 'bg-gray-100 text-gray-700' };
  const outcome = call.outcome
    ? OUTCOME_CONFIG[call.outcome] || { label: call.outcome, color: 'bg-gray-100 text-gray-700' }
    : null;
    ...
    <div className="flex items-center justify-between mb-4">
      ...
      <div className="flex items-center gap-2">
        {call.leadGenerated && call.generatedLeadId && (
          <button
            onClick={() => onViewLead(call.generatedLeadId!)}
  ...
  <div className="flex flex-wrap items-center gap-4 mb-4 p-3 bg-white rounded-lg border text-sm">
    ...
    {call.sentiment && (
      <div className="flex items-center gap-2">
        ...
        <span className={`px-1.5 py-0.5 rounded text-xs font-medium ${getSentimentColor(call.sentiment)}`}>
          {call.sentiment}
        </span>
      </div>
    )}
    ...
    {call.recordingUrl && (
      <div className="flex items-center gap-2 ml-auto">
        <audio ref={audioRef} src={call.recordingUrl} onEnded={onAudioEnded} className="hidden" />
        <button
...
export const TranscriptPanel: React.FC<TranscriptPanelProps> = ({ call }) => (
...
export const InfoPanel: React.FC<InfoPanelProps> = ({ call }) => (
...
export const ContactCard: React.FC<ContactCardProps> = ({ call }) => (
  <div className="bg-white rounded-lg border">
...
export const OutcomeCard: React.FC<OutcomeCardProps> = ({ call, onViewLead }) => {
  const status = STATUS_CONFIG[call.status] || { label: call.status, color: 'bg-gray-100 text-gray-700' };
  const outcome = call.outcome
    ? OUTCOME_CONFIG[call.outcome] || { label: call.outcome, color: 'bg-gray-100 text-gray-700' }
    : null;
      ...
      <div className="p-3 space-y-2">
        ...
        {call.sentiment && (
          <div className="flex justify-between items-center">
            ...
            <span className={`px-2 py-0.5 rounded text-xs font-medium ${getSentimentColor(call.sentiment)}`}>
              {call.sentiment}
            </span>
          </div>
        )}
...
export const QualificationCard: React.FC<QualificationCardProps> = ({ call }) => (
  <div className="bg-white rounded-lg border">
      ...
      {call.qualification && Object.keys(call.qualification).length > 0 ? (
        <div className="space-y-2">
          {Object.entries(call.qualification)
            .filter(([key]) => !EXCLUDED_QUALIFICATION_KEYS.includes(key))
            .map(([key, value]) => {
              const displayValue = formatQualificationValue(value);
              const formattedKey = formatQualificationKey(key);
              const isLong = isLongTextKey(key, displayValue);

              return (
                <div key={key} className={isLong ? 'flex flex-col gap-0.5' : 'flex justify-between items-start'}>
                  <span className="text-gray-500 capitalize text-sm">{formattedKey}</span>
                  <span className={`font-medium text-gray-900 text-sm ${isLong ? 'text-left bg-gray-50 p-2 rounded' : 'text-right max-w-[55%]'}`}>
                    {displayValue}
                  </span>
                </div>
              );
...
export const CampaignCard: React.FC<CampaignCardProps> = ({ campaign, onViewCampaign }) => (
  <div className="bg-white rounded-lg border">
