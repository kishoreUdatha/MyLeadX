...
export const OutboundCallsPage: React.FC = () => {
  ...
  const [activeTab, setActiveTab] = useState<'campaigns' | 'calls'>('campaigns');
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const [campaignsRes, callsRes, analyticsRes] = await Promise.all([
              ...
              {calls.length === 0 ? (
                <tr>
                  <td colSpan={7} className="px-4 py-6 text-center text-gray-500">
                    ...
                    <p className="text-xs">No calls yet</p>
                  </td>
                </tr>
              ) : (
                calls.map((call) => (
