      ...
      <div className="bg-white rounded-xl shadow-lg p-6">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-4">
          {plans.map((plan) => {
            ...
            const isUpgrade = plans.indexOf(plan) > plans.findIndex(p => p.id === subscription?.planId);
...
function UsageCard({ label, used, limit }: { label: string; used: number; limit: number }) {
  ...
  const color = percentage >= 90 ? 'bg-red-500' : percentage >= 70 ? 'bg-amber-500' : 'bg-primary-500';
...
function AddOnModal({
  ...
  // Add-on pricing per plan (matches backend ADD_ONS)
  const addOnPricing: Record<string, Record<string, number>> = {
  ...
  const getCurrentPrice = () => {
    const pricing = addOnPricing[selectedAddOn];
    if (!pricing) return 0;
    return pricing[planId] || pricing['starter'] || 0;
  };
  ...
  const handlePurchase = async () => {
    ...
    setLoading(true);
    try {
      console.log('Purchasing add-on:', selectedAddOn, 'quantity:', quantity);

      const result = await subscriptionService.purchaseAddOn(
        selectedAddOn as any,
        quantity
      );
      ...
      const errorMessage = err.response?.data?.message || err.message || 'Failed to initiate purchase';
      toast.error(errorMessage);
      setLoading(false);
    }
  };

  const selectedAddonDetails = addOns.find(a => a.id === selectedAddOn);
  const pricePerUnit = getCurrentPrice();
