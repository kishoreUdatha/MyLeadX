/**
 * One-shot: render the Daily Manager Digest with mock data so you can preview
 * exactly what the email looks like. No DB calls.
 *
 * Run:  npx ts-node backend/scripts/sample-digest-html.ts
 * Out:  backend/tmp/sample-digest.html  (open in any browser)
 */
...
const mock: DigestData = {
  ...
  managerRows: [
    ...
    },
    {
      ...
      branchName: 'Pune — Koregaon Park',
      teamSize: 4,
      calls: 70,
      connected: 41,
      connectRatePct: (41 / 70) * 100,
      interested: 7,
      converted: 1,
    },
    {
...
const outFile = path.join(outDir, 'sample-digest.html');
fs.writeFileSync(outFile, html, 'utf8');

console.log(`Wrote ${html.length} bytes to ${outFile}`);
console.log(`Open it: start "" "${outFile}"`);
